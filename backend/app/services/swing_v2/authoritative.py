...
_SESSION_READ_CACHE: dict[str, Any] | None = None
...
def _refresh_snapshot(reason: str, *, deadline: datetime | None = None) -> dict[str, Any]:
    """Return the latest persisted snapshot for the final decision window.

    Swing V2 never creates an Angel One client or calls Angel REST APIs.  The
    snapshot is the same shared, provider-independent artifact the Intraday
    desk consumes.  When the deadline has arrived or the shared refresh lock is
    held, fall back to the last persisted snapshot so the final scan can still
    be produced instead of poisoning the book with a MISSED_ORDER_WINDOW
    cash-hold.
    """
    try:
        from .market_data import read_snapshot
        ...
        if not acquired:
            ...
            return read_snapshot()
        try:
            # The producer never performs Swing strategy work; the shared
            # snapshot is refreshed by the existing scheduler path.
            return read_snapshot()
        finally:
...
def _quote_observations(symbols: list[str], now: datetime) -> dict[str, dict[str, Any]]:
    """Fetch canonical quote observations for post-decision paper evidence.

    Uses the neutral shared market-data interface only — never an Angel One
    client, subscription, or REST call.  Ask is the best available sell-side
    proxy (NSE/Dhan canonical LTP) and askDepth is the reported trade volume.
    """
    if not symbols:
        return {}
    try:
    ...
    # captured before a potentially slow data request.
    stamp = datetime.now(timezone.utc).isoformat()
    for symbol, quote in quotes.items():
        if not isinstance(quote, dict):
            continue
        try:
            ask = float(quote.get("ltp") or quote.get("open") or 0)
            depth = int(float(quote.get("tradeVolume") or 0))
        except (TypeError, ValueError):
            continue
        if ask <= 0:
            continue
        result[str(symbol).upper()] = {
            ...
            "ask": ask,
            "askDepth": depth,
            "open": float(quote.get("open") or ask),
            "high": float(quote.get("high") or ask),
            "low": float(quote.get("low") or ask),
            "close": float(quote.get("close") or ask),
            "source": str(quote.get("quoteProvider") or "NEUTRAL_FEED"),
        }
    return result
...
def _manage_open_positions(ledger: SwingLedger, now: datetime, cfg: SwingV2Config) -> None:
    ...
    try:
        from .market_data import historical_bars
        ...
        for symbol in wanted:
            ...
            for state in symbol_states:
                ...
                try:
                    ...
                    continue
            # Recover every bar since the last durable event instead of only
            # 12 minutes; one throttled/failed symbol must not discard good
            # paths for the remaining positions — its batched quote is fallback.
            start = min(cursors) if cursors else now - timedelta(minutes=12)
            try:
                bars = historical_bars(symbol, "ONE_MINUTE", 240)
...
def run_authoritative_cycle(*, now: datetime | None = None, force: bool = False) -> dict[str, Any]:
    ...
    with _LOCK:
        ...
        occupied = intraday_occupied_symbols(day)
...
def get_authoritative_session(*, live: bool = False) -> dict[str, Any]:
    ...
    with _LOCK:
        if _SESSION_READ_CACHE is None or now - _SESSION_READ_CACHE_AT >= _SESSION_READ_TTL:
            _SESSION_READ_CACHE = _session()
...
def authoritative_eod_report(for_date: date) -> dict[str, Any]:
    ...
    now = monotonic_time.monotonic()
    with _LOCK:
        cached = _EOD_READ_CACHE.get(cache_key)
        if cached and now - float(cached.get("_cachedAt", 0.0)) < _SESSION_READ_TTL:
    ...
    with _LOCK:
        _EOD_READ_CACHE[cache_key] = {"_cachedAt": monotonic_time.monotonic(), "payload": copy.deepcopy(report)}
    return report
