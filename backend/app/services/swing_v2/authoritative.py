...
def _session(scan=None,*,now=None):
 ...
 rows=[_position_row(r,marks.get(str(r.get("symbol") or "").upper())) for r in active]; state=_read_json(_state_path()); state=state if str(state.get("sessionDate") or "")==day else {}; effective=scan if isinstance(scan,dict) else state.get("scan"); local=now.time().replace(tzinfo=None); start=_clock(cfg.decision_start_ist); freeze_clock=_clock(cfg.decision_freeze_ist); freeze=local>=freeze_clock; before_decision=local<start; in_window=start<=local<freeze_clock; finalized=bool(state.get("selectionFinalized")); locked_today=any(str(r.get("sessionDate") or "")==day for r in positions); blocked=bool((effective or {}).get("blocked")); cash=finalized and not locked_today; realized=sum(float(r.get("realizedPnl") or 0) for r in rows); unreal=sum(float(r.get("unrealizedPnl") or 0) for r in rows); diagnostics=_entry_hunt_diagnostics(effective,snapshot) or {}; regime_cap={"NORMAL":5,"DEFENSIVE":2,"HALT_NEW_LONGS":0}.get(str((effective or {}).get("regime") or snapshot.get("swingV2Regime") or ""),0); capacity=min(cfg.max_positions,regime_cap); diagnostics={**diagnostics,"diagnosticPhase":"WAITING_FOR_DECISION_WINDOW" if before_decision and not finalized else ("V2_DECISION_SCAN" if in_window and not finalized else "V2_FINALIZED"),"refreshError":state.get("refreshError"),"lastScanAt":state.get("lastScanAt"),"maxPositions":cfg.max_positions,"regimePositionCap":regime_cap,"openPositions":len(active),"availableSlots":max(0,capacity-len(active)),"qualifiedCount":(effective or {}).get("qualifiedCount"),"selectedCount":(effective or {}).get("selectedCount"),"correlationEvidencePairs":(effective or {}).get("correlationEvidencePairs"),"scanBlockReason":(effective or {}).get("blockReason")}
