...
class SwingV2Config:
    ...
    microcap_mode: str = "SATELLITE"
    microcap_priority_weight: float = 0.20
    max_positions: int = 5
    ...
    def validate(self) -> None:
        if self.strategy_id != "SWING_2S_MOMENTUM_V2": raise ValueError("SWING_STRATEGY_ID must remain SWING_2S_MOMENTUM_V2")
        if self.mode not in {"SHADOW", "PAPER"}: raise ValueError("SWING_V2_MODE must be SHADOW or PAPER before live promotion")
        if self.authority not in {"V1", "V2"}: raise ValueError("SWING_STRATEGY_AUTHORITY must be V1 or V2")
        if self.authority == "V2" and (not self.enabled or self.mode != "PAPER"): raise ValueError("V2 authority requires SWING_V2_ENABLED=true and SWING_V2_MODE=PAPER")
        if self.microcap_mode not in {"DISABLED", "SHADOW", "SATELLITE"}: raise ValueError("SWING_MICROCAP_MODE must be DISABLED, SHADOW or SATELLITE")
        if not 0 < self.microcap_priority_weight <= 0.20: raise ValueError("SWING_MICROCAP_PRIORITY_WEIGHT must be in (0, 0.20]")
        if not 1 <= self.max_positions <= 5: raise ValueError("SWING_MAX_POSITIONS must be 1..5")
        if self.max_overnights not in {1, 2}: raise ValueError("SWING_MAX_OVERNIGHTS must be 1 or 2")
        if self.nav <= 0: raise ValueError("SWING_CAPITAL must be positive")
        ...
        freeze, expire = _clock(self.decision_freeze_ist, "SWING_DECISION_FREEZE_IST"), _clock(self.order_expire_ist, "SWING_ORDER_EXPIRE_IST")
        mandatory = _clock(self.mandatory_exit_ist, "SWING_MANDATORY_EXIT_IST")
        if not start < freeze < cutoff < expire: raise ValueError("decision clocks must satisfy start < freeze < entry cutoff < order expiry")
        if mandatory > expire: raise ValueError("mandatory exit must not exceed order expiry clock")
        if self.live_promotion: raise ValueError("SWING_LIVE_PROMOTION is not permitted by this implementation")
...
def load_config() -> SwingV2Config:
    ...
    c = SwingV2Config(
        ...
        entry_cutoff_ist=os.getenv("SWING_ENTRY_CUTOFF_IST", "15:15"), decision_start_ist=os.getenv("SWING_DECISION_START_IST", "09:45"), decision_freeze_ist=os.getenv("SWING_DECISION_FREEZE_IST", "15:00"), order_expire_ist=os.getenv("SWING_ORDER_EXPIRE_IST", "15:20"), mandatory_exit_ist=os.getenv("SWING_MANDATORY_EXIT_IST", "15:15"),
