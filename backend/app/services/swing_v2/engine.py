...
from .lifecycle import apply_corporate_action, evaluate_position
...
def process_corporate_action(
    ...
    *,
    ...
    if not events:
        raise ValueError(f"unknown position {position_id}")
    before = materialize_position(events)
    after = apply_corporate_action(before, ratio)
    ts = timestamp or datetime.now().astimezone().isoformat()
    ledger.append(
        idempotency_key=f"{position_id}:{ts}:CORPORATE_ACTION_ADJUSTED",
        decision_id=str(before["decisionId"]),
        position_id=position_id,
        symbol=str(before["symbol"]),
        session_date=str(before["sessionDate"]),
        event_type=EventType.CORPORATE_ACTION_ADJUSTED,
        event_timestamp=ts,
        payload=after,
    )
    return materialize_position(ledger.events(position_id=position_id))
