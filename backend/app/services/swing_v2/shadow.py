...
def build_shadow_v2(rows: list[dict[str, Any]], *, universe_coverage: float = 0.0, regime: str = "REGIME_UNRATED", final_lock: bool = False, occupied_symbols: set[str] | None = None, existing_positions: list[dict[str, Any]] | None = None, correlations: dict[tuple[str, str], float] | None = None, persist_events: bool = False, now: datetime | None = None, config: SwingV2Config | None = None, apply_coverage_hysteresis: bool = True) -> dict[str, Any]:
    ...
    tier, coverage_risk_multiplier, hysteresis = _coverage_tier(tradable_coverage, cfg, session_date=session_date, apply_hysteresis=apply_coverage_hysteresis)
    _LOG.info("Swing V2 coverage fresh=%d denominator=%d coverage=%.4f denominator_source=ACTIVE_PAPER_HUNT_ROWS tier=%s raw_tier=%s", len(fresh_rows), denominator, tradable_coverage, tier, hysteresis.get("rawTier"))

    funnel = {"universe_size": len(rows), "evaluated_count": len(tradable_rows), "fresh_count": len(fresh_rows), "stale_excluded_count": len(stale_rows), "coverage_pct": round(tradable_coverage * 100.0, 4), "coverage_denominator": denominator, "coverage_numerator": len(fresh_rows), "coverage_denominator_source": "ACTIVE_PAPER_HUNT_ROWS", "gate_tier": tier, "coverage_risk_multiplier": coverage_risk_multiplier, "candidates_in": len(fresh_rows), "qualified_out": 0, "block_reason": None, "hysteresis": hysteresis, "universe": len(rows), "freshData": len(fresh_rows), "tradable": 0, "safetyPass": 0, "setupPass": 0, "expectancyPass": 0, "portfolioPass": 0, "locked": 0, "filled": 0, "blocked": False}
    if tier == "BLOCK":
        funnel.update(blocked=True, block_reason="UNIVERSE_COVERAGE_BELOW_90PCT", candidates_in=0)
        return {**base, "blocked": True, "retryable": True, "blockReason": "UNIVERSE_COVERAGE_BELOW_90PCT", "coverage": tradable_coverage, "tradableCoverage": tradable_coverage, "coverageTier": tier, "coverageRiskMultiplier": 0.0, "coverageDenominatorSource": "ACTIVE_PAPER_HUNT_ROWS", "freshRows": len(fresh_rows), "staleExcludedRows": len(stale_rows), "shadowCoverage": shadow_coverage, "candidates": [], "qualifiedCount": 0, "funnel": funnel}
    ...
    for raw in rows:
        ...
        if symbol in stale_symbols or not fresh_ok:
            rejected.append({"symbol": symbol, "reasonCodes": sorted(set(fresh_reasons or ["STALE_EXCLUDED_BY_COVERAGE_TIER"])), "qualificationStage": "FRESHNESS"})
            continue
        ...
        expected = row.get("expectedNetR"); calibrated = expected is not None and str(row.get("expectedNetRStatus") or "").upper() == "CALIBRATED"; expectancy_ok = calibrated and float(expected) >= cfg.min_expected_net_r
    ...
    qualified = assign_segment_percentiles(qualified); funnel["qualified_out"] = len(qualified); funnel["qualified"] = len(qualified)
    ...
    funnel["portfolioPass"] = len(selected); funnel["locked"] = len(selected) if final_lock else 0
    if persist_events:
    ...
    return {**base, "generatedAt": now.isoformat(), "regime": regime, "regimeRiskScale": regime_risk_scale, "coverageTier": tier, "coverageRiskMultiplier": coverage_risk_multiplier, "effectiveRiskScale": effective_risk_scale, "coverage": tradable_coverage, "tradableCoverage": tradable_coverage, "coverageDenominatorSource": "ACTIVE_PAPER_HUNT_ROWS", "shadowCoverage": shadow_coverage, "sourceSnapshotId": snapshot_id, "candidateCount": len(fresh_rows), "qualifiedCount": len(qualified), "selectedCount": len(selected), "candidates": selected, "rejected": rejected + portfolio["rejected"], "funnel": funnel, "portfolioInitialRisk": portfolio["portfolioInitialRisk"], "gapStressLoss": portfolio["gapStressLoss"], "cash": portfolio["cash"]}
