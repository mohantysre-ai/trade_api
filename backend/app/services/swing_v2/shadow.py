...
def build_shadow_v2(
    ...
    *,
    ...
    if not cfg.enabled:
        return {**base, "candidates": [], "funnel": {"universe": len(rows)}, "tradableCoverage": 0.0, "shadowCoverage": 0.0}
    ...
    tradable_covered = sum(1 for r in tradable_rows if evaluate_freshness(r, final_lock=final_lock, now=now)[0])
    ...
    for raw in rows:
        ...
        # Net-of-friction expected reward
    ...
    cutoff_parts = [int(p) for p in getattr(cfg, "entry_cutoff_ist", "14:45").split(":")]
    cutoff_time = time(cutoff_parts[0], cutoff_parts[1])
    if ist_now.time().replace(tzinfo=None) >= cutoff_time:
        for item in portfolio["selected"]:
            portfolio["rejected"].append({"symbol": item.get("symbol"), "portfolioRejectReason": "ENTRY_CUTOFF_AFTER_1445_IST"})
        portfolio["selected"] = []
        selected = []
    ...
    return {**base, "generatedAt": now.isoformat(), "regime": regime, "riskScale": risk_scale, "coverage": universe_coverage, "tradableCoverage": tradable_coverage, "shadowCoverage": shadow_coverage, "sourceSnapshotId": snapshot_id, "candidateCount": len(rows), "qualifiedCount": len(qualified), "selectedCount": len(selected), "candidates": selected, "rejected": rejected + portfolio["rejected"], "funnel": {**funnel, "topRejectionReasons": sorted(({"reason": key, "count": value} for key, value in reason_counts.items()), key=lambda item: item["count"], reverse=True)[:10]}, "portfolioInitialRisk": portfolio["portfolioInitialRisk"], "gapStressLoss": portfolio["gapStressLoss"], "cash": portfolio["cash"]}
