...
def _ticker_stock_row(payload: dict[str, Any], ticker: str) -> dict[str, Any]:
    ...
    if isinstance(quote, dict) and quote:
        ...
        merged = {**merged, **quote}
        if q_intra.get("data_source") in ("candles", "daily_candles"):
...
def _build_dynamic_selection_reason(stock: dict[str, Any], score: float | None) -> str:
    ...
    momentum_text = f"delta {delta:.2f}% with score {score:.1f}" if score is not None else f"delta {delta:.2f}%; score unavailable"
    liquidity_text = (
