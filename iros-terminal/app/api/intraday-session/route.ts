...
/** GET /api/intraday-session — locked basket + MTM (proxies FastAPI). */
export async function GET() {
  try {
    const { data, cacheStatus } = await cachedBackendJson(
      "intraday-session",
      `${BACKEND_URL}/api/intraday-session`,
      4_000
    );
    ...
    try {
      const { data, cacheStatus } = await cachedBackendJson(
        "intraday-session-lite",
        `${BACKEND_URL}/api/intraday-session?live=0`,
        2_000
      );
      return NextResponse.json(data, { headers: liveCacheHeaders(cacheStatus) });
    } catch (err) {
