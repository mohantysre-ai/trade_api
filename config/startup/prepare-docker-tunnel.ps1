...
# calendar.sigq.in → host.docker.internal:8088; other hostnames → frontend:3000 by default.
...
foreach ($m in [regex]::Matches($hostYaml, '(?ms)^\s*-\s*hostname:\s*(\S+)\s*\r?\n\s*service:\s*(\S+)\s*$')) {
  ...
  if ($svc -match '127\.0\.0\.1:8088' -or $svc -match 'localhost:8088') {
    $svc = "http://host.docker.internal:8088"
...
if ($ingressPairs.Count -eq 0) {
  [void]$ingressPairs.Add([pscustomobject]@{ Hostname = "calendar.sigq.in"; Service = "http://host.docker.internal:8088" })
