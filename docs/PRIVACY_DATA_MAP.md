...
| Model providers | OpenRouter and its routed models, Google Gemini, optional self-hosted OmniRoute and whichever upstreams it routes; other endpoint when configured | Ticker, selected market facts/news excerpts, prompt, generated response, quota/error metadata |
...
Do not list every OpenRouter- or OmniRoute-routed model as a separately guaranteed recipient: the actual downstream model may change under free-router/failover configuration. Treat each gateway and selected model host as a variable processing chain and review provider settings for training, retention, and region before sending personal data.
